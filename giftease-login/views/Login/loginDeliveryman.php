...
-->
      <form method="POST" action="?type=deliveryman&action=login" class="center">
        <input type="email" name="email" placeholder="Email" required>
        <input type="password" name="password" placeholder="Password" required
        ...
        <a href="?action=login&type=giftWrapper" class="btn">GiftWrapper Login</a>
        <a href="?action=login&type=admin" class="btn">Admin Login</a>
        <a href="?action=signup&type=deliveryman" class="btn">No account? Sign up</a>
      </form>
      <?php if (!empty($error)): ?>
