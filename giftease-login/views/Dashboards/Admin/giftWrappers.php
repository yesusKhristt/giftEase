        ...
        <div class="main-content">
            <section id="customers" class="page active">
                <div class="page-header">
                    ...
                    <p class="subtitle">Manage Gift Wrapper Accounts</p>
                </div>
                ...
                <div style="margin: 20px 0; display: flex; gap: 10px; align-items: center;">
                    <form id="searchForm" method="GET" style="display: flex; gap: 10px; flex: 1;">
                        ...
                        <input type="hidden" name="action" value="dashboard/giftWrappers">
                        <input type="text" id="searchInput" name="search" placeholder="Search by name or email..." value="<?php echo htmlspecialchars($search); ?>" style="flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                        ...
                        <?php if (!empty($search)): ?>
                            <a href="?controller=admin&action=dashboard/giftWrappers" style="padding: 10px 15px; background-color: #999; color: white; border: none; border-radius: 5px; text-decoration: none; cursor: pointer;"><i class="fas fa-times"></i> Clear</a>
                        <?php endif; ?>
                    </form>
                </div>
                ...
                <div style="margin-bottom: 15px; color: #666; font-size: 14px;">
                    Showing <?php echo count($paginatedGiftWrappers); ?> of <?php echo $totalItems; ?> gift wrappers
                </div>
                ...
                <div class="staff-cards-container">
                    <?php 
                      ...
                      $pending = [];
                      $verified = [];
                      foreach ($paginatedGiftWrappers as $row) {
                        if ($row['verified']) {
                          $verified[] = $row;
                        } else {
                          $pending[] = $row;
                      ...
                      $paginatedGiftWrappers = array_merge($pending, $verified);
                      foreach ($paginatedGiftWrappers as $row): 
                    ?>
                        <div class="staff-card <?php echo $row['verified'] ? 'verified' : 'pending'; ?>">
                            ...
                            <div class="staff-header">
                                <div class="staff-info">
                                    ...
                                    <p class="shop-name"><i class="fas fa-gift"></i>Experience: <?php echo htmlspecialchars($row['years_of_experience']); ?> years</p>
                                </div>
                            ...
                            <div class="staff-actions">
                                <?php if ($row['verified']): ?>
                                    <a class="btn-unverify" href="?controller=admin&action=dashboard/giftWrappers/unverify/<?php echo htmlspecialchars($row['giftWrapper_id']); ?>">
                                        <i class="fas fa-times"></i> Unverify
                                    </a>
                                <?php else: ?>
                                    <a class="btn-verify" href="?controller=admin&action=dashboard/giftWrappers/verify/<?php echo htmlspecialchars($row['giftWrapper_id']); ?>">
                ...
                <div style="margin-top: 30px; display: flex; justify-content: center; gap: 10px; align-items: center; flex-wrap: wrap;">
                    ...
                    for ($i = $startPage; $i <= $endPage; $i++) {
                        ...
                        echo '<a href="?controller=admin&action=dashboard/giftWrappers&page=' . $i . ((!empty($search) ? '&search=' . urlencode($search) : '')) . '" style="padding: 8px 12px; background-color: ' . $bgColor . '; color: ' . $color . '; border: ' . $border . '; border-radius: 4px; text-decoration: none; cursor: pointer;">' . $i . '</a>';
                    }
                    ?>  
                </div>
