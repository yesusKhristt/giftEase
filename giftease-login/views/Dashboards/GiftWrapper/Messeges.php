...
<?php
    ...
    foreach ($myMessages as $row) {
        ...
        // sent = 0 means received from client (unread = not yet read by giftwrapper)
                ...
                ${msg.attachments.length ? `
                    <div class="attachments">
                        ${msg.attachments.map(f =>
                            `<img src="resources/uploads/giftWrapper/attatchments/${f}" class="imageBox">`
                        ).join('')}
                    </div>
                ` : ''}
...
function markAsRead(clientId) {
    fetch(`?controller=giftWrapper&action=dashboard/messeges/markRead/${clientId}`, {
        method: 'POST',
...
function sendMessage() {
    ...
    fetch(`?controller=giftwrapper&action=dashboard/messeges/send/${currentClientId}`, {
        method: 'POST',
        body: formData
    })
    .then(r => r.json())
